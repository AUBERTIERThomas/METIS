...
real,dimension(:),allocatable :: tl,Y1,resultat,r1,rephcp, repvcp,repprp,repprp2
...
write(*,*) " Programme d'étalonnage à la boule des dispositif slingram fréquentiel"
write(*,*) "le calcul est basé sur keller et friknesht"
write(*,*) "le champ est considéré uniforme sur le volume de la boule"
...
if (ioerr==0) then
  ...
  read (*,*) num_buck
  write(*,*) "dispositif calculés : Perp (CS), Perp (DualEM) HCP, VCP"
endif
...
do ib=1,nb_cfg
  if (nb_col>1) then
    ...
    enddo
    form_col=trim(form_col)//','//st2i(nb_col+1:nb_col+1)//'(F10.3)'
  else
    tit_col=trim(tit_col)//' '//lab_cfg(ib)
    form_col=trim(form_col)//',F10.3'
  endif
enddo
...
!      WRITE(*,form_col)y,REPprp,repprp2,rephcp,repvcp
  ...
  do
    ...
    write(*,*)'attention le déplacement est selon y, l''axe du dispositif'
    READ(*,*)X,Y,Z
  ...
  ! calcul VCP
    ...
    !WRITE(*,form_col)y,REPprp,repprp2,rephcp,repvcp
      write(7,form_col) y,REPprp,repprp2,rephcp,repvcp
        ...
        y=y+n 
      END do
      WRITE(*,*) 'on continue avec une autre g�om�trie initiale? (oui:1)'
      READ(*,*) i
      IF(i/=1) then
        exit
      else
